...
from src.content.loader.base import BaseLoader
...
class MagicLoader(BaseLoader):
    """
    Magic loader that automatically detects content type and loads content
    using the appropriate LangChain document loader.

    The loader is initialized with an LLM instance and can be reused
    to load multiple pieces of content.
    """

    def __init__(self, llm: BaseChatModel):
        """
        Initialize the magic loader with an LLM.

        Args:
            llm: LangChain chat model instance for summarization
        """
        self.llm = llm
    ...
    def load(self, link: str) -> Content:
        ...
        raw_docs = self._load_raw_content(link, format_type)

        if not raw_docs:
            raise ValueError(f"No content could be loaded from: {link}")

        # 3. Extract source metadata
        source = self._create_source(link, format_type, raw_docs)

        # 4. Generate summary using LLM
        summary = generate_summary(raw_docs, self.llm)

        # 5. Combine all document content
        raw_content = "\n\n".join([doc.page_content for doc in raw_docs])

        # 6. Extract metadata
        metadata = self._extract_metadata(link, format_type, raw_docs)
    ...
    def _load_raw_content(self, link: str, format_type: Format) -> List[Document]:
        """
    ...
    def _create_source(self, link: str, format_type: Format, documents: List[Document]) -> Source:
        """
    ...
    def _extract_metadata(self, link: str, format_type: Format, documents: List[Document]) -> Dict[Any, Any]:
        """
